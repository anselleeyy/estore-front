                  ...
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <div>
                        ...
                        <!--价格-->
                        <div class="price1">¥ {{ item.price.toFixed(2) }}</div>
                      </div>
                    </div>
                  </div>
                </div>
...
<!--                  <div class="delete-choose-goods" @click="delItem(item.itemId)">删除选中的商品</div>-->
              ...
              <div class="shipping">
                ...
                <y-button :classStyle="checkNum > 0 && submit?'main-btn':'disabled-btn'"
                          ...
                          :text="checkoutNow" @btnClick="checkout"></y-button><!-- @btnClick="checkout" -->
              </div>
            </div>
          </div>
        </div>
    ...
    editCart (type, item) {
      if (type && item) {
        ...
        if (type === 'check') {
          ...
          this._cartEdit(this.userId, itemId, itemNum, newChecked)
        }
      } else {
        console.log('缺少所需参数')
      }
    },
    EditNum (itemNum, itemId, checked) { // 数量
      this._cartEdit(this.userId, itemId, itemNum, checked)
    },
    // 删除整条购物车
    cartdel (itemId) {
      console.log(itemId)
